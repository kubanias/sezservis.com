...
class lessc {
    ...
    protected function evaluate($exp) {
        ...
        //echo '<pre>' . print_r($exp, true) . '</pre><br/>';
        if (!($exp[4] == 1 && $exp[5] == 1 && isset($exp[2][2]) && isset($exp[3][2]) && $exp[2][2] != $exp[3][2])) {	// Skip "calc"
...
class lessc_parser {
    ...
    protected function argumentDef(&$args, &$isVararg) {
        ...
        while (true) {
            ...
            if ($this->$method($value)) {				// THIS IS NOT AN ERROR!!! The result will be returned to $value
                if ($value[0] == "variable") {
                    $arg = array("arg", $value[1]);
                    $ss = $this->seek();

                    if ($this->assign() && $this->$method($rhs)) {	// THIS IS NOT AN ERROR!!! The result will be returned to $rhs
                        $arg[] = $rhs;
                    } else {
                        $this->seek($ss);
                        if ($this->literal("...")) {
    ...
    protected function genericList(&$out, $parseItem, $delim="", $flatten=true) {
        $s = $this->seek();
        $items = array();
        while ($this->$parseItem($value)) {		// THIS IS NOT AN ERROR!!! The result will be returned to $value
            $items[] = $value;
            if ($delim) {
                if (!$this->literal($delim)) break;
            }
